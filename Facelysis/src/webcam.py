...
def start_webcam(model_emotion, model_gender, window_size, window_name='live', update_time=50):
    ...
    video_feed = cv2.VideoCapture("nvcamerasrc ! video/x-raw(memory:NVMM), width=(int)1280, height=(int)720,format=(string)I420, framerate=(fraction)30/1 ! nvvidconv flip-method=0 ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink")
...
if __name__ == '__main__':
    ...
    if (choice == 'y'):
        ...
        run_loop = True
        window_name = "Static (press ESC to exit)"
        print("Default path is set to data/sample/")
        print("Type q or quit to end program")
        while run_loop:
            path = "/home/nvidia/Desktop/Facelysis/src/data/sample/"
